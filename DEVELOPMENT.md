...
**回复生成特点（v2.1.4+）：**
- **视觉转述**：将视觉模型识别结果以"你看到的画面：..."的方式传入，模拟 AI 亲眼看到场景
- 自动获取最近 5 条对话历史，参照上下文生成回复
- **保留角色设定**：图片分析信息作为用户消息的一部分传入，不覆盖系统提示词
  - LLM 保留原有的角色性格设定和语言风格约束
  - 保持与对话上下文一致的语言风格、格式和回复长度习惯
  - 回复尽量简短（通常1-2句话即可），避免收到图片后产生超长回复
  - 让回复自然融入之前的对话风格中，提供更连贯的对话体验
- 明确要求 LLM 不要提及"图片"、"照片"、"截图"等词汇，就像是自己亲眼看到这个场景一样
